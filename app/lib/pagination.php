...
class Pagination
{
    ...
    protected $options = array(
        ...
        'class_ul'                      => 'pagination justify-content-center',
    ...
    */
    ...
    public function build_li_element($class, $text, $is_link = false, $page_number = 0)
    {
        if($is_link == false)
        {
            $this->links_html .= '<li class="'.$class.'"><span class="page-link">'.$text.'</span></li>'.PHP_EOL;
        }
        else
        {
            $this->links_html .= '<li class="'.$class.'"><a class="page-link" href="'.$this->build_link_url($page_number).'">'.$text.'</a></li>'.PHP_EOL;
        }
    }
    ...
    protected function build_links_next_last()
    {
        ...
        if($this->options['show_links_prev_next'] == true)
        {
            ...
            {
                $this->links_html .= '<li  class="'.$this->options['class_dead_links'].'"><span class="page-link">'.$this->options['text_next'].'</span></li>'.PHP_EOL;
            }
            elseif($this->current_page != $this->total_pages)
            {
                $this->links_html .= '<li class="'.$this->options['class_live_links'].'"><a class="page-link" href="'.$this->build_link_url($this->current_page + 1).'">'.$this->options['text_next'].'</a></li>'.PHP_EOL;
            }
        ...
        if($this->options['show_links_first_last'] == true)
        {
            ...
            {
                $this->links_html .= '<li class="'.$this->options['class_dead_links'].'"><span class="page-link">'.$this->options['text_last'].'</span></li>'.PHP_EOL;
            }
            elseif($this->current_page != $this->total_pages)
            {
                $this->links_html .= '<li class="'.$this->options['class_live_links'].'"><a class="page-link" href="'.$this->build_link_url($this->total_pages).'">'.$this->options['text_last'].'</a></li>'.PHP_EOL;
            }
